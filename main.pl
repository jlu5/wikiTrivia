...
score_answer(UserAnswer, RHSLabel, 1) :-
  string_lower(UserAnswer, LowerUserAnswer),
  string_lower(RHSLabel, LowerRHSLabel),
  ==(LowerUserAnswer, LowerRHSLabel),
  format("Correct! The answer is ~w \n", [RHSLabel]).
score_answer(UserAnswer, RHSLabel, 0) :-
...
ask_questions(AllRows, FormatString, RemainingQuestions, CurrentScore) :-
  ...
  % Score is 0 or 1
  score_answer(UserAnswer, RHSLabel, Score),
